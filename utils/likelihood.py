...
try:
    ...
    class _WeibullFallback:
        @staticmethod
        def log_hazard(log_params, times, respective_times=True):
            del respective_times
            log_scale = log_params[:, 0]
            log_shape = log_params[:, 1]
            times = times.clamp(min=1e-8)
            return log_shape - log_scale + (torch.exp(log_shape) - 1.0) * (torch.log(times) - log_scale)

        @staticmethod
        def _cumulative_hazard(log_params, times, respective_times=True):
            del respective_times
            log_scale = log_params[:, 0]
            log_shape = log_params[:, 1]
            times = times.clamp(min=1e-8)
            return torch.exp(torch.exp(log_shape) * (torch.log(times) - log_scale))

    weibull = _WeibullFallback()
...
def loglik_surv_piecewise(batch_data, list_type, theta, normalization_params, n_generated_dataset):
    ...
    denom = torch.clamp(data_max - data_min, min=1e-8)
    T_surv_scaled = ((T_surv - data_min) / denom).clamp(min=0.0, max=1.0 - 1e-7)
    ...
    eps = 1e-8
    breaks = torch.cat((
        torch.tensor([interval[0] for interval in intervals], dtype=T_surv_scaled.dtype, device=data.device),
        torch.tensor([intervals[-1][1]], dtype=T_surv_scaled.dtype, device=data.device),
    ))
    bin_idx = torch.bucketize(T_surv_scaled, breaks[1:], right=False).flatten().clamp(max=K - 1)  # shape: (n,)
    ...
    density_T = torch.nan_to_num(density_T, nan=1.0 / K, posinf=1.0 / K, neginf=1.0 / K).clamp(min=eps)
    density_C = torch.nan_to_num(density_C, nan=1.0 / K, posinf=1.0 / K, neginf=1.0 / K).clamp(min=eps)
    density_T = density_T / density_T.sum(dim=1, keepdim=True).clamp(min=eps)
    density_C = density_C / density_C.sum(dim=1, keepdim=True).clamp(min=eps)
    ...
    w = ((T_surv_scaled - t0) / bin_width).clamp(min=0.0, max=1.0)
    S_T_t = ((1 - w) * S0_T + w * S1_T).clamp(min=eps)
    S_C_t = ((1 - w) * S0_C + w * S1_C).clamp(min=eps)
    ...
    # Generate samples from softmax bin probability masses.
    def sample_from_density(density, intervals, data_min, data_max):

        batch_size = density.shape[0]
        dev = density.device
        # Interval widths and start points
        intervals_tensor = torch.tensor(intervals, dtype=density.dtype, device=dev)  # shape: (K, 2)
        starts = intervals_tensor[:, 0]  # shape: (K,)
        widths = intervals_tensor[:, 1] - intervals_tensor[:, 0]  # shape: (K,)

        # 1. Softmax outputs are interval probability masses already.
        probs = torch.nan_to_num(density, nan=0.0, posinf=0.0, neginf=0.0).clamp(min=eps)
        probs = probs / probs.sum(dim=1, keepdim=True).clamp(min=eps)  # normalize just in case
